...
class TestFLT010:
    ...
    Uses perf ID type (length=10) since digits_group_limit=5
    requires ID length >= 10."""

    async def test_first_equals_last_rejected(
        self, client, perf_id_type, perf_id_length, service_config, validate_id
    ):
        ...
        resp = await validate_id(client, perf_id_type, bad_id)
        assert resp.status_code == 200
...
class TestFLT012:
    ...
    async def test_restricted_number_rejected(
        self, client, id_type_1, validate_id
    ):
        pytest.skip(
...
class TestFLT013:
    ...
    Uses perf ID type (length=10) since 142857 is 6 digits."""

    async def test_cyclic_number_rejected(
        self, client, perf_id_type, perf_id_length, validate_id
    ):
...
class TestFLT016:
    ...
    async def test_boundary_sequence_allowed(
        ...
        resp = await validate_id(client, id_type_1, candidate)
        assert resp.status_code == 200
...
class TestFLT018:
    ...
    async def test_non_numeric_rejected(
        self, client, id_type_1, validate_id, bad_id
    ):
        resp = await validate_id(client, id_type_1, bad_id)
